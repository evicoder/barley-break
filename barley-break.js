var cell_empty = 'c15'; //Переменная с нынешней пустой клеткой
	...
	window.onload = function(){ //функция выполнится только после загрузки страницы. Я решил, что здесь будет перемешка
	  var steps = [1, 4, -1, -4]; //массив со значениями для шагов
	  var step = 0; //нынешний шаг
	   for (var i = 0; i < 200; i++) //вот это самое "200" - это будет кол-во шагов в перемешшке.
	   { 
	   	 var c_empt = window.cell_empty.substr(1); //для удобства засунем в переменную id клетки, где сейчас пустая костяшка. Без "c"
	...
	function check_move(move) { //функция с проверкой, доступен ли шаг куда-либо
	  var cempty = window.cell_empty.substr(1); //присваиваем ид клетки с пустой костяшкой в переменную
	  var razn = move-cempty; //разница между шагом куда нажато и пустой клеткой. Ходить можно только в те, которые рядом. А это те, у которых разница равна +-1 и +-4
	  var check = false; //создаём переменную, которая говорит, можно ли шагать. Сначала делаем так, что нельзя
	...
	function complete() { //функция, проверяющая, собрана ли головоломка
	  var compl = true; //переменная, в которой сказано, выполнено ли. Сначала думаем, что да
	   for (var i = 0; i <= 15; i++) { //проверяем все клетки
    ...
    function move(cell, perem) { //функция для передвижения. Имеет два аргумента. В первом ССЫЛКУ НА ОБЪЕКТ, по которому кликнули. Второй нужен для того, чтобы показать, что сейчас перемешка. Это нужно для того, чтобы не появилось сообщение о собранности головоломки, если рандом соберет её в процессе перемешки
	  var cempty = window.cell_empty.substr(1); //пустую клетку окунаем в переменную для удобства
         if (check_move(cell.id.substr(1))) { //если можно сделать шаг, куда было нажато, то делаем шаг
		  var emp = document.getElementById(window.cell_empty); //Пустая клетка СЕЙЧАС
		  var cell_n = cell.id; //ИД нажатой ячейки	
		  var temp; //переменная чисто для того, чтобы поменять местами две клетки
